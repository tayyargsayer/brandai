...
class Authenticate:
    ...
    def login(self, location: str='main', max_concurrent_users: Optional[int]=None,
        ...
        """
        if fields is None:
            fields = {'Form name':'Eye Authentic', 'Username':'Kullanıcıs Adı', 'Password':'Şifre',
                      'Login':'Login'}
        if location not in ['main', 'sidebar']:
            # Temporary deprecation error to be displayed until a future release
            raise DeprecationError("""Likely deprecation error, the 'form_name' parameter has been
    ...
    def logout(self, button_name: str='Çıkış Yap', location: str='sidebar', key: Optional[str]=None):
        """
    ...
    def register_user(self, location: str='main', pre_authorization: bool=True,
        ...
        """
        if fields is None:
            fields = {'Form name':'Kayıt Formu', 'Name': 'İsim Soyisim', 'Email':'E-mail', 'Username':'Kullanıcı Adı',
                      'Password':'Şifre', 'Repeat password':'Şifre Tekrar',
                      'Register':'Kayıt Ol!'}
